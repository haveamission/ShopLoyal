...
class Card extends Component {
    ...
    lightestColor(colors) {
        ...
        colors.forEach(function (color) {
            let lightness = lightOrDark(color);
            if (lightness > hspHighest && lightness < 200) {
                hspHighest = lightness;
                highestColor = color;
            }
        });
        let hsla = hexToRgbA(highestColor);
    ...
    lightestColorGen() {
        if (typeof this.props.merchant !== "undefined") {
            ...
            getColors(this.props.merchant.logo).then(colors => {
                ...
                let colorsHSLA = colors.map(color => color.hsl());
                let colorsHex = colors.map(color => color.hex());
                self.lightestColor(colorsHex);
            });
